...
internal actual fun AutoMemory?.zeroP3(): P3 = P3Impl(ge_p3_identity, this!!)


internal actual fun AutoMemory?.BasePoint(): P3 = BasePointImpl(this!!)
...
actual fun AutoMemory?.ge_fromfe_frombytes_vartime(s: FeLeUInt): P2 {
    val p2 = P2Impl.allocStruct(this!!)
    ge_fromfe_frombytes_vartime(p2.ptr, s.ubytes.refTo(0))
    return P2Impl(p2, this)
}
...
open class P3Impl(val p3: ge_p3, autoMem: NativePlacement): P3, ComparableEcPointAutoMem(autoMem) {
    override fun negate(): P1P1  = (this.autoMem as AutoMemory).zeroP3().sub(toCached())
...
//        return P3Impl(res, autoMem)
    ...
    override fun scalarMultiplyP2(scal: EcScalar): P2 {
        val res: ge_p2 = P2Impl.allocStruct(autoMem)
        ge_scalarmult(res.ptr, scal.ubytes.refTo(0), p3.ptr)
        return P2Impl(res, autoMem)
    }
