...
class MultiBulletproof(
    ...
    private val N: Int = 2.0.pow(NEXP).toInt() // number of bits in amount range (so amounts are 0..2^(N-1))
    ...
    init {
        autoMem {
            ...
            val MAXM = 2.0.pow(MAXEXP.toDouble()).toInt()
            Gi = Array<P3>(MAXM * N) {
...
*/
    ...
    fun randomProof(mExp: Int, aMem: AutoMemory?): ProofTuple {
        val M = 2.0.pow(mExp).toInt()

        val rando = Rand.get()
        val amounts = Array(M) {
            var amount = -1L
            while (amount > 2.0.pow(N) - 1 || amount < 0L) {
    ...
    fun VERIFY(proofs: Array<ProofTuple>): Boolean {
        autoMem {
            ...
            val maxMN = 2.0.pow(maxLength.toDouble()).toInt()
            ...
            for (p in proofs.indices) {
                ...
                val M = 2.0.pow(logMN.toDouble()).toInt() / N
...
//TODO sq()                (z.sq().mul(InnerProduct(VectorPowers(EcScalar.SC_ONE, M * N), VectorPowers(y, M * N))))
                ...
                for (i in 0 until M * N) {
                    ...
                    for (j in logMN - 1 downTo 0) {
                        ...
                        val basePow = 2.0.pow(j).toInt() //TODO!!! assumes we don't get too big
                        if (index / basePow == 0)
                        // bit is zero
                        {
...
fun VectorExponentCustom(
    ...
    require(a.size == A.size && b.size == B.size && a.size == b.size)
